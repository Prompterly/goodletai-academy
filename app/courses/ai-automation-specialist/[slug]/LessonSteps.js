...
export default function LessonSteps({ lesson }) {
  ...
  const completeLesson = () => {
    const completedLessons = JSON.parse(localStorage.getItem('completedLessons_automation') || '[]')
    if (!completedLessons.includes(lesson.lessonNumber)) {
      completedLessons.push(lesson.lessonNumber)
      localStorage.setItem('completedLessons_automation', JSON.stringify(completedLessons))
    }
    if (lesson.hasMilestone && lesson.milestone?.badgeName) {
      ...
      if (!badges.find(b => b.name === lesson.milestone.badgeName)) {
        ...
        localStorage.setItem('earnedBadges_automation', JSON.stringify(badges))
      }
    }
  ...
  if (showMilestone) {
    return (
      <MilestoneScreen
        ...
        onContinue={() => window.location.href = '/courses/ai-automation-specialist'}
      />
    )
  }
    ...
    return (
      <div className="page-enter" style={{
        ...
        <div style={{ maxWidth: '500px', textAlign: 'center', color: 'white', width: '100%' }}>
          ...
          <a className="milestone-button btn-next" href="/courses/ai-automation-specialist" style={{
            display: 'inline-block',
            background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
            color: 'white',
            padding: '16px 40px',
  ...
  if (!step) {
    return (
      <div style={{ textAlign: 'center', padding: '100px 20px' }}>
        ...
        <a href="/courses/ai-automation-specialist" style={{ color: '#667eea' }}>← Back</a>
      </div>
    )
  }
