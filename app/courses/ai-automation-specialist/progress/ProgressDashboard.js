...
const allBadges = [
  { name: 'Automation Thinker', emoji: '🤖', description: "You've built the mental foundation of an AI automation builder.", earnedAfter: 5 },
  { name: 'No-Code Builder', emoji: '⚙️', description: "You've built real, working automations with no code.", earnedAfter: 10 },
  { name: 'GPT Creator', emoji: '🧠', description: "You designed, built, and deployed a Custom GPT from scratch.", earnedAfter: 15 },
  { name: 'Workflow Architect', emoji: '🏗️', description: "You've designed and built complete end-to-end AI workflows.", earnedAfter: 20 },
  { name: 'Prompt Chain Master', emoji: '🔗', description: "You can build prompt chains that produce expert-level AI outputs.", earnedAfter: 25 },
  { name: 'AI Automation Specialist', emoji: '🎓', description: "You have completed the AI Automation Specialist course.", earnedAfter: 30 }
]
...
export default function ProgressDashboard({ lessons }) {
  ...
  useEffect(() => {
    const saved = JSON.parse(localStorage.getItem('completedLessons_automation') || '[]')
    const badges = JSON.parse(localStorage.getItem('earnedBadges_automation') || '[]')
    ...
    <div style={{ minHeight: '100vh', fontFamily: 'Arial, sans-serif', background: '#f8fafc' }}>
      ...
      <header style={{
        ...
        <h1 style={{ fontSize: '2.2rem', marginBottom: '10px', fontWeight: 'bold' }}>
          Your AI Automation Progress
        </h1>
        <p style={{ opacity: 0.85, fontSize: '1.1rem' }}>
          Track your journey from beginner to AI Automation Specialist
        </p>
      </header>
      ...
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '50px 20px' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            height: '140px',
            borderRadius: '50%',
            background: `conic-gradient(#10a37f ${progressPercent * 3.6}deg, #edf2f7 0deg)`,
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            margin: '0 auto 25px'
            ...
            <div style={{
              ...
              <span style={{ fontSize: '2.2rem', fontWeight: '800', color: '#10a37f' }}>
                {progressPercent}%
              </span>
            </div>
          </div>
          ...
          {nextLesson && (
            <a
              href={`/courses/ai-automation-specialist/${nextLesson.slug.current}`}
              style={{
            ...
            <div key={weekNum} style={{ marginBottom: '30px' }}>
              ...
              {weeks[weekNum].map((lesson) => {
                ...
                return (
                  <a
                    key={lesson._id}
                    href={`/courses/ai-automation-specialist/${lesson.slug.current}`}
                    style={{
                      display: 'flex',
                      alignItems: 'center',
                      gap: '15px',
